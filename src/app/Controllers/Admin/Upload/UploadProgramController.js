...
class UploadProgramController {
    // [GET] /admin/upload-program (admin)
        ...
        try {
            ...
            if (request.query.isPending) {
                if (request.query.createdAt) {
                    request.query.createdAt = moment(request.query.createdAt).startOf('isoDate').subtract(1, 'day').format('YYYY-MM-DD');
                }
                if (request.query.updatedAt) {
                    request.query.updatedAt = moment(request.query.updatedAt).startOf('isoDate').subtract(1, 'day').format('YYYY-MM-DD');
                }
            }
    ...
    // [GET] /admin/uploadEdit/:id
    async detail(request, response, next) {
        const params = request.params;
        const errors = [];
        try {
            const uploadData = await programEditModel
                .findById({
                    _id: params.id,
                })
                .populate([
            ...
            return logger.status200(response, system.success, '', uploadData);
        } catch (error) {
            errors.push(error.message);
    ...
    // [POST] /admin/uploadEdit/approval
    async updateStatus(request, response, next) {
        ...
        try {
            if (user && user.userType) {
                ...
                if (!isValidOperation) {
                    session.endSession();
                    return logger.status400(response, uploadConstant.invalidData, errors);
                }
                //allow status
                const allowedStatus = [
                    ...
                    switch (formData.programCurrentStatus) {
                        ...
                        case constants.PROGRAM_STATUS.APPROVAL:
                            if (
                                ...
                                await editData.save(session);
                                // Update program status to approval
                                program.programCurrentStatus =
                                    constants.PROGRAM_STATUS.APPROVAL;
                                ...
                                if (
                                    ...
                                    await programModel.updateOne(
                                        {
                                            _id: program.programChildrenSeasonData
                                                .parentID,
                                        },
                                        {
                                            programCurrentStatus:
                    ...
                    switch (formData.programCurrentStatus) {
                        ...
                        case constants.PROGRAM_STATUS.APPROVAL:
                            if (
                                ...
                                // Socket for episode
                                adminService.notiStatusEpisodeOfflineEDIT(
                                    editData,
                                    formData.programCurrentStatus
                                );
    ...
    // [GET] /admin/upload-program/status
    ...
    async update(request, response, next) {
        ...
        try {
            ...
            const upload = await programEditModel
    ...
    // [GET] /admin/uploadEdit/list-soft-delete
    async listSoftDelete(request, response, next) {
        ...
        try {
            ...
            const uploadProDel = await businessQuery.handle(
                programEditModel,
                request,
                { path: 'userID', select: ['userName', 'userEmail'] },
                selectFields
            ...
            return logger.status200(response, system.success, '', uploadProDel);
        } catch (error) {
            errors.push(error.message);
            return logger.status500(response, error, errors);
        }
    ...
    // [GET] /admin/uploadEdit/request-delete
    async listDelete(request, response, next) {
        ...
        try {
            ...
            delete request.query.id;
            const uploadReqDel = await businessQuery.handle(
                programEditModel,
                request,
                { path: 'userID', select: ['userName', 'userEmail'] },
                selectFields
            );
            return logger.status200(response, system.success, '', uploadReqDel);
        } catch (error) {
            errors.push(error.message);
            return logger.status500(response, error, errors);
        }
    ...
    // [POST] /admin/upload-program/history/:id
    async addHistory(request, response, next) {
        ...
        try {
            ...
            const history = new HistoryProgramModel({
                ...
                programID: params.id,
                typeProgram: constants.TYPE_PROGRAM_HISTORY.UPLOAD_PROGRAM,
            ...
            if (checkProgram.isResultLetter === false) {
                ...
                await checkProgram.save(session);
            }

            const dataProgram = await programEditModel
                .findOne({ programID: params.id })
                .populate({
                    path: 'userID',
                })
                .sort({ createdAt: -1 });
            ...
            const messageProgram = new messageModel({
                receiver: dataProgram.userID._id.toString(),
                sender: constants.DATA_SUPER_ADMIN.ID,
                receiverEmail: dataProgram.userID.userEmail,
    ...
    // [GET] /admin/upload-program/history/:id
    async getHistory(request, response, next) {
        ...
        try {
            ...
            if (
                ...
                !checkProgram.programSeasonChild && checkProgram.verifyDenial === null
            ) {
...
module.exports = new UploadProgramController();
