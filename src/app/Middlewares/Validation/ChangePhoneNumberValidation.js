...
const changePhoneNumberValidation = async (request, response, next) => {
	...
	try {
		...
		const isValidOperation = keyData.every((update) => {
			return userConstant.allowedChangePhoneNumber.includes(update);
		});

		if (!isValidOperation) {
			const fields = (keyData.filter(item => !userConstant.allowedChangePhoneNumber.includes(item))).join(', ')
...
module.exports = changePhoneNumberValidation;
