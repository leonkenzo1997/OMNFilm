...
const videoSettingValidation = async (request, response, next) => {
  ...
  try {
    ...
    const isValidOperation = keyData.every((update) => {
      return userConstant.checkFieldsVideoSetting.includes(update);
    });

    if (!isValidOperation) {
      const fields = keyData
        .filter((item) => !userConstant.checkFieldsVideoSetting.includes(item))
        .join(", ");
...
module.exports = videoSettingValidation;
