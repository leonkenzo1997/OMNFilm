...
const homeSetValidation = async (request, response, next) => {
    ...
    try {
        ...
        const isValidOperation = keyData.every((update) => {
            return homeSetConstant.allowedUpdates.includes(update);
        });

        if (!isValidOperation) {
            const fields = (keyData.filter(item => !homeSetConstant.allowedUpdates.includes(item))).join(', ')
...
module.exports = homeSetValidation;
