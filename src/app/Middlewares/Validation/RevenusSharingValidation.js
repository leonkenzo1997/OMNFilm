...
const revenusSharingValidation = async (request, response, next) => {
    ...
    try {
        ...
        const isValidOperation = keyData.every((update) => {
            return revenusSharingConstant.allowedUpdates.includes(update);
        });

        if (!isValidOperation) {
            const fields = (keyData.filter(item => !revenusSharingConstant.allowedUpdates.includes(item))).join(', ')
...
module.exports = revenusSharingValidation;
