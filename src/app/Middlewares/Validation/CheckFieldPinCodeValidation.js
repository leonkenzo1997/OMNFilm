...
const CheckFieldPinCodeValidation = async (request, response, next) => {
	...
	try {
		...
		const isValidOperation = keyData.every((update) => {
			return userConstant.checkFieldPinCode.includes(update);
		});

		if (!isValidOperation) {
			const fields = (keyData.filter(item => !userConstant.checkFieldPinCode.includes(item))).join(', ')
...
module.exports = CheckFieldPinCodeValidation;
