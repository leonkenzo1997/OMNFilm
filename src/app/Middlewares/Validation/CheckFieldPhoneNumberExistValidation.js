...
const checkFieldPhoneNumberValidation = async (request, response, next) => {
	...
	try {
		...
		const isValidOperation = keyData.every((update) => {
			return userConstant.checkPhoneNumberExistField.includes(update);
		});

		if (!isValidOperation) {
			const fields = keyData.filter((item) => !userConstant.checkPhoneNumberExistField.includes(item)).join(', ');
...
module.exports = checkFieldPhoneNumberValidation;
