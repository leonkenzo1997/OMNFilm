...
const checkUpdateUserValidation = async (request, response, next) => {
	...
	try {
		...
		const isValidOperation = keyData.every((update) => {
			return userConstant.checkUpdateUser.includes(update);
		});

		if (!isValidOperation) {
			const fields = (keyData.filter(item => !userConstant.checkUpdateUser.includes(item))).join(', ')
...
module.exports = checkUpdateUserValidation;
