...
const changePasswordValidation = async (request, response, next) => {
	...
	try {
		...
		const isValidOperation = keyData.every((update) => {
			return userConstant.allowedChangePassword.includes(update);
		});

		if (!isValidOperation) {
			const fields = (keyData.filter(item => !userConstant.allowedChangePassword.includes(item))).join(', ')
