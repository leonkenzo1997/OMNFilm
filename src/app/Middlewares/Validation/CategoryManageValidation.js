...
const categoryManageValidation = async (request, response, next) => {
    ...
    try {
        ...
        const isValidOperation = keyData.every((update) => {
            return categoryManageConstant.allowedUpdates.includes(update);
        });

        if (!isValidOperation) {
            const fields = (keyData.filter(item => !categoryManageConstant.allowedUpdates.includes(item))).join(', ')
...
module.exports = categoryManageValidation;
