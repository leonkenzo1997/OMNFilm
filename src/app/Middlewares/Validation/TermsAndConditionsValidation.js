...
const termsAndConditionsValidation = async (request, response, next) => {
    ...
    try {
        ...
        const isValidOperation = keyData.every((update) => {
            return termsAndConditionsConstant.allowedUpdates.includes(update);
        });

        if (!isValidOperation) {
            const fields = (keyData.filter(item => !termsAndConditionsConstant.allowedUpdates.includes(item))).join(', ')
...
module.exports = termsAndConditionsValidation;
