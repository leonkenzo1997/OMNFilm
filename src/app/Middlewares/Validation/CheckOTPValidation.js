...
const CheckOTPValidation = async (request, response, next) => {
	...
	try {
		...
		const isValidOperation = keyData.every((update) => {
			return userConstant.checkOTPField.includes(update);
		});

		if (!isValidOperation) {
			const fields = (keyData.filter(item => !userConstant.checkOTPField.includes(item))).join(', ')
...
module.exports = CheckOTPValidation;
