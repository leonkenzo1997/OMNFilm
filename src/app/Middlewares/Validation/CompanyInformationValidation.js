...
const companyInformationConstant = require('../../Constant/CompanyInformation/CompanyInforConstant');
const logger = require('../../Constant/Logger/loggerConstant');

const companyInformationValidation = async (request, response, next) => {
    ...
    try {
        ...
        const isValidOperation = keyData.every((update) => {
            return companyInformationConstant.allowedUpdates.includes(update);
        });

        if (!isValidOperation) {
            const fields = (keyData.filter(item => !companyInformationConstant.allowedUpdates.includes(item))).join(', ')
